...
namespace CAS.Expression
{
    ...
    {
        Exp _A, _B;
        public Div(Exp A, Exp B) { _A = A; _B = B; }
        public override Exp Diff(string Var)
        {
            Exp _A_Simp = _A.Simplify();
            Exp _B_Simp = _B.Simplify();
            return ((_A_Simp.Diff(Var) * _B_Simp) - (_A_Simp * _B_Simp.Diff(Var))) / (_B_Simp * _B_Simp);
        }
        public override Exp Evaluate()
        {
            return new Div(_A, _B); ;
        }
        public override Exp Simplify()
        {
            ...
            if (_A_Simp.IsConst())
            {
                ...
                if (_B_Simp.IsConst())
                {
                    if (_B_Simp.IsNull()) { return new Value(float.NaN); }
        ...
        {
            Exp _A_Simp = _A.Simplify(); 
            return _A_Simp.IsNull();
        }
        public override bool IsConst()
        {
            Exp _A_Simp = _A.Simplify();
            Exp _B_Simp = _B.Simplify();
            if (_A_Simp.IsNull()) { return true; }
